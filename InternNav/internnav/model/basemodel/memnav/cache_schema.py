...
def _strict_indices(name: str, value, expected: np.ndarray) -> np.ndarray:
    ...
    if not np.array_equal(indices, expected):
        ...
        raise ValueError(
            f"cache {name} does not match {KEYFRAME_POLICY}: "
...
def validate_cache_pair(
    ...
    *,
    ...
    _require_equal("keyframe_policy", values["keyframe_policy"], KEYFRAME_POLICY)
    ...
    if interval < 1:
        raise ValueError(f"cache keyframe_interval must be positive, got {interval}")
    ...
    expected_cam = camera_keyframe_indices(num_frames, scale, interval)
    ...
    cam_indices = _strict_indices(
        "cam_frame_indices", camera["cam_frame_indices"], expected_cam
    )
    if validate_payload:
        _validate_payload_lengths(
            aggregator, camera, num_frames, anchor_indices, cam_indices
...
def validate_cache_files(
    ...
    *,
    ...
    """
    metadata = _SCHEMA_FIELDS | {"kv_cache_sliding_window"}
