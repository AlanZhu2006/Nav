...
**02:52 更新：** 用户要求恢复后，已定位并修复 CSV float32 往返解析问题，未放宽复算容差。
74 段几何与 overfit 结果复用；两臂、三种子完整训练和独立复算均完成。验证平均位置误差的
三种子平均为视觉 1.0704 m、视觉+XYZ 1.0396 m，未形成替换 CEC 的证据，无新导航 SR。
详见 [ANCHOR_RELATION_GEOMETRY_RESULT_20260907.md](ANCHOR_RELATION_GEOMETRY_RESULT_20260907.md)。
以下保留此前归档时的暂停状态，不代表当前仍在失败或运行。
