"""Fixed-RGB production-memory cost benchmark in fresh, monitored processes.

Two preselected history lengths, all three navigation configurations, three
...
def freeze(args):
    ...
    for repeat in range(3):
        for case in cases:
            for j in range(3):
                tasks.append(dict(index=len(tasks), case=case['id'], repeat=repeat, mode=MODES[(j+repeat)%3]))
    save(args.out / 'plan.json', dict(cases=cases, tasks=tasks, seed=20260913,
        ...
        protocol='Real MemNavAgent, identical ordered RGB, no NavDP/Habitat; 3 fresh-process repetitions',
...
def worker(args):
    ...
    agent = MemNavAgent(str(MEM_CKPT), str(ROOT/'InternNav'), buffer_root=str(args.out/'buffer'),
        ...
        memory_mechanism=mode, flow_gate='auto' if mode=='legacy' else 'off')
...
def run(args):
    ...
    for task in plan['tasks']:
        ...
        env = dict(os.environ, PYTHONUNBUFFERED='1', OMP_NUM_THREADS='4', OPENBLAS_NUM_THREADS='4',
            ...
            MEMNAV_WINDOW='32' if task['mode']=='legacy' else '64', MEMNAV_NUM_SCALE='8',
