...
parser.add_argument(
    ...
    help="Habitat scene asset; the default runtime recomputes its navmesh",
)
parser.add_argument(
    "--pinned_navmesh", type=str, default="",
    help=("optional content-addressed navmesh loaded verbatim instead of "
          "runtime recomputation"),
)
parser.add_argument(
    "--expected_pinned_navmesh_sha256", type=str, default="",
    help="required lowercase SHA-256 receipt when --pinned_navmesh is used",
)
parser.add_argument(
    "--scene_identity",
    type=str,
...
if bool(args.pinned_navmesh) != bool(args.expected_pinned_navmesh_sha256):
    parser.error(
        "--pinned_navmesh and --expected_pinned_navmesh_sha256 are atomic")
if args.expected_pinned_navmesh_sha256 and not re.fullmatch(
        r"[0-9a-f]{64}", args.expected_pinned_navmesh_sha256):
    parser.error("--expected_pinned_navmesh_sha256 must be lowercase 64-hex")
...
def main():
    ...
    if args.pinned_navmesh:
        pinned_path = Path(args.pinned_navmesh)
        if not pinned_path.is_file():
            raise FileNotFoundError(f"pinned navmesh missing: {pinned_path}")
        digest = hashlib.sha256()
        with pinned_path.open("rb") as handle:
            for block in iter(lambda: handle.read(8 << 20), b""):
                digest.update(block)
        if digest.hexdigest() != args.expected_pinned_navmesh_sha256:
            raise RuntimeError("pinned navmesh SHA-256 receipt mismatch")
        sim = make_sim(
            args.scene, str(pinned_path), agent_radius=args.agent_radius,
