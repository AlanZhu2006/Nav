...
class LightGluePointMatcher:
    ...
    def match_paths(self, reference_path: Path, query_path: Path, *,
        ...
        reference_image = self.load_image(str(reference_path)).to(self.device)
        ...
        reference_points = map_raw_points_to_lingbot_pad(
            raw_reference,
            raw_height=int(reference_image.shape[-2]),
            raw_width=int(reference_image.shape[-1]),
        ...
        return {
            ...
            "reference_raw_hw": [
                int(reference_image.shape[-2]), int(reference_image.shape[-1])],
            "query_raw_hw": [
