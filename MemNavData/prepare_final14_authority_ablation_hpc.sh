...
PREPARATION_RECEIPT=${PREPARATION_RECEIPT:-MemNavData/FINAL14_CEC_AUTHORITY_ABLATION_PREPARATION_RECEIPT_20260828.json}
SUBMISSION_RECEIPT=${SUBMISSION_RECEIPT:-MemNavData/FINAL14_CEC_AUTHORITY_ABLATION_SUBMISSION_RECEIPT_20260828.json}
...
REMOTE_HAB_REQUESTS_VENDOR=$(remote \
  "/scratch/lg154/conda-envs/habitat/bin/python -c 'import sysconfig; print(sysconfig.get_paths()[\"purelib\"] + \"/pip/_vendor\")'" |
  tr -d '\r')
[[ "${REMOTE_HAB_REQUESTS_VENDOR}" == /scratch/*/pip/_vendor ]] || \
  fail "could not resolve Habitat vendored requests path"

[[ "${SUBMIT}" =~ ^[01]$ ]] || fail "SUBMIT must be 0 or 1"
[[ -z "${UPSTREAM_AFTEROK}" || "${UPSTREAM_AFTEROK}" =~ ^[0-9]+$ ]] || \
  fail "UPSTREAM_AFTEROK must be empty or one numeric Slurm job id"
[[ "${PREPARATION_RECEIPT}" == MemNavData/*.json \
   && "${SUBMISSION_RECEIPT}" == MemNavData/*.json ]] || \
  fail "receipt paths must be JSON files under MemNavData"
...
PYTHONDONTWRITEBYTECODE=1 PYTHONPATH="${ROOT}:${ROOT}/MemNavData" \
  "${LOCAL_PY}" -m unittest -q \
    MemNavData.test_policy_agent_graph MemNavData.test_router_candidates
...
remote "set -euo pipefail; \
  ...
  test -r '${REMOTE_HAB_REQUESTS_VENDOR}/requests/__init__.py'"
...
prepare_receipt=${PREPARATION_RECEIPT}
...
smoke_dependency=""
if [[ -n "${UPSTREAM_AFTEROK}" ]]; then
  smoke_dependency="--dependency=afterok:${UPSTREAM_AFTEROK} --kill-on-invalid-dep=yes"
fi
smoke_job=$(remote "source '${repair_root}/MemNavData/slurm_safe_submit.sh'; safe_sbatch --lint-fatal --parsable ${smoke_dependency} --qos=gpu48 --array=0 --export='${common},RUN_ROOT=${smoke_root},SMOKE=1,MAX_STEPS=80' '${gpu_script}'" | job_id)
...
"${LOCAL_PY}" - "${receipt}" "${repair_root}" "${receipt_sha}" \
  ...
  "${analysis_job}" "${UPSTREAM_AFTEROK}" <<'PY'
import json, sys
path,bundle,digest,run,smoke,smoke_job,formal,analysis,upstream=sys.argv[1:]
payload={
 ...
 "upstream_afterok_job": upstream or None,
