...
files=(
  ...
  MemNavData/slurm_hm3d_episodic_path_field_gate.sbatch
  MemNavData/submit_hm3d_episodic_path_field_gate_remote.sh
...
"${PY}" -m py_compile \
  ...
  NavDP/baselines/memnav/policy_agent.py \
  NavDP/baselines/memnav/memnav_server.py
bash -n MemNavData/slurm_hm3d_episodic_path_field_gate.sbatch \
  MemNavData/run_hm3d_fullmono_server_scene.sh \
  MemNavData/prepare_hm3d_episodic_path_field_gate_bundle.sh \
  MemNavData/submit_hm3d_episodic_path_field_gate_remote.sh

mkdir -p "${PARENT}"
stage=$(mktemp -d "${PARENT}/hm3d_epf_gate.partial.XXXXXX")
for path in "${files[@]}"; do
...
(
  ...
  env PYTHONDONTWRITEBYTECODE=1 PYTHONNOUSERSITE=1 \
    PYTHONPATH="${stage}" "${PY}" -c \
      'import MemNavData.lingbot_pnp_localization; import NavDP.baselines.memnav.policy_agent'
...
payload={
 'schema_version':'hm3d_episodic_path_field_gate_bundle_v3_20260902',
 'local_git_head_context':sys.argv[2],
 'scientific_scope':'proof-once monotone visual-route deployment gate',
...
target=${PARENT}/hm3d_episodic_path_field_gate_${receipt_sha:0:16}
