...
PROTOCOL_SHA=8231222ad2a91658987dd640d20697d1bf78614d0f9939090f215f0dfe95f391
fail() { echo "ABORT: $*" >&2; exit 2; }

files=(
  MemNavData/hm3d_longrange_dense_query_gate_protocol_v2_20260903.json
  MemNavData/run_hm3d_longrange_dense_query_gate_history.py
  MemNavData/analyze_hm3d_longrange_dense_query_gate.py
  MemNavData/independent_verify_hm3d_longrange_dense_query_gate.py
  MemNavData/slurm_hm3d_longrange_dense_query_gate.sbatch
  MemNavData/slurm_hm3d_longrange_dense_query_gate_result.sbatch
  MemNavData/submit_hm3d_longrange_dense_query_gate_remote.sh
  MemNavData/test_hm3d_longrange_dense_query_gate.py
...
cd "${ROOT}"
[[ "$(sha256sum MemNavData/hm3d_longrange_dense_query_gate_protocol_v2_20260903.json | awk '{print $1}')" == "${PROTOCOL_SHA}" ]] || \
  fail "frozen dense-query protocol changed"
...
for template in \
    MemNavData/slurm_hm3d_longrange_dense_query_gate.sbatch \
    MemNavData/slurm_hm3d_longrange_dense_query_gate_result.sbatch; do
  lint_sbatch_template "${template}" || fail "Slurm template lint failed"
done

mkdir -p "${PARENT}"
stage=$(mktemp -d "${PARENT}/hm3d_dense_query_gate.partial.XXXXXX")
...
payload={
 'schema_version':'hm3d_longrange_dense_live_query_gate_bundle_v2_20260903',
 'local_git_head_context':sys.argv[2],
 'scientific_scope':'consumed dense causal query-motion mechanism gate',
 'protocol_sha256':sys.argv[3],
 'selected_histories':[0,1,6],
...
target=${PARENT}/hm3d_longrange_dense_query_gate_${receipt_sha:0:16}
