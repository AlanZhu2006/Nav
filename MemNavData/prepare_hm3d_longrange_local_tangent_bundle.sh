...
files=(
  MemNavData/LONG_RANGE_LOCAL_TANGENT_ATTRIBUTION_PROTOCOL_20260903.md
  MemNavData/LONG_RANGE_LOCAL_TANGENT_ATTRIBUTION_PROTOCOL_20260903.md.sha256
  MemNavData/longrange_local_tangent_attribution.py
  MemNavData/eval_hm3d_longrange_local_tangent_attribution.py
  MemNavData/run_hm3d_longrange_local_tangent_attribution.py
  MemNavData/slurm_hm3d_longrange_local_tangent_attribution.sbatch
  MemNavData/submit_hm3d_longrange_local_tangent_attribution_remote.sh
  MemNavData/test_longrange_local_tangent_attribution.py
...
(cd MemNavData && sha256sum -c --quiet \
  LONG_RANGE_LOCAL_TANGENT_ATTRIBUTION_PROTOCOL_20260903.md.sha256)
[[ "$(sha256sum MemNavData/LONG_RANGE_LOCAL_TANGENT_ATTRIBUTION_PROTOCOL_20260903.md | awk '{print $1}')" == "${PROTOCOL_SHA}" ]] || \
  fail "v2 protocol changed"
env PYTHONPATH="${ROOT}/MemNavData:${ROOT}" "${PY}" -m pytest -q \
  MemNavData/test_longrange_local_tangent_attribution.py
...
bash -n \
  ...
  MemNavData/slurm_hm3d_longrange_local_tangent_attribution.sbatch \
  MemNavData/submit_hm3d_longrange_local_tangent_attribution_remote.sh \
  MemNavData/prepare_hm3d_longrange_local_tangent_bundle.sh

mkdir -p "${PARENT}"
stage=$(mktemp -d "${PARENT}/hm3d_longrange_tangent_v2.partial.XXXXXX")
for path in "${files[@]}"; do
...
(
  ...
  env PYTHONDONTWRITEBYTECODE=1 PYTHONNOUSERSITE=1 \
    PYTHONPATH="${stage}/MemNavData:${stage}" "${PY}" -m pytest -q \
      MemNavData/test_longrange_local_tangent_attribution.py
  env PYTHONDONTWRITEBYTECODE=1 PYTHONNOUSERSITE=1 \
  ...
  env PYTHONDONTWRITEBYTECODE=1 PYTHONNOUSERSITE=1 \
    ...
    "${NAVDP_PY}" -m unittest -q MemNavData.test_navdp_memory_replay
...
payload={
 'schema_version':'hm3d_longrange_local_tangent_bundle_v2_20260903',
 'local_git_head_context':sys.argv[2],
 'scientific_scope':'one consumed long-range privileged mechanism diagnosis',
 'protocol_sha256':sys.argv[3],
 'population_indices':[32], 'distance_bin':'30_to_50_m',
 'query_role':'revisit',
 'arms':['oracle_chord_mixed_realized','oracle_tangent_mixed_realized',
...
target=${PARENT}/hm3d_longrange_local_tangent_${receipt_sha:0:16}
