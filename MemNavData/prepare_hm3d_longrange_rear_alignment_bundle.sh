...
files=(
  ...
  MemNavData/prepare_hm3d_longrange_rear_alignment_bundle.sh
  ...
  MemNavData/monocular_route_tangent_contract.py
...
cd "${ROOT}"
[[ "$(sha256sum MemNavData/hm3d_longrange_rear_alignment_protocol_20260904.json | awk '{print $1}')" == "${PROTOCOL_SHA}" ]] || \
  fail "frozen rear-alignment protocol changed"
...
lint_sbatch_template MemNavData/slurm_hm3d_longrange_rear_alignment.sbatch || \
  fail "evaluation Slurm template lint failed"
lint_sbatch_template MemNavData/slurm_hm3d_longrange_rear_alignment_result.sbatch || \
  fail "result Slurm template lint failed"

mkdir -p "${PARENT}"
stage=$(mktemp -d "${PARENT}/hm3d_rear_alignment.partial.XXXXXX")
...
payload={
 'schema_version':'hm3d_longrange_rear_alignment_bundle_v1_20260904',
...
target=${PARENT}/hm3d_longrange_rear_alignment_${receipt_sha:0:16}
