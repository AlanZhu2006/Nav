"""Audit all preplanned native W16/W64 cost repetitions."""
import argparse
import hashlib
import json
    ...
    for case in plan['cases']:
        for window in (16,64):
            ...
            chosen = [r for r in rows if r['task']['case']==case['id'] and r['task']['mode']==mode
                and r['task']['dense_window']==window]
            if not chosen:
                continue
            assert len({r['task']['repeat'] for r in chosen}) == len(chosen)
            groups[case['id']+'/'+mode+'/w'+str(window)] = dict(repeats=len(chosen),dense_window=window,
