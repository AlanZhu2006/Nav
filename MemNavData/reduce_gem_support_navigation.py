...
from run_gem_support_navigation import ARMS, check_plan, load, sha
...
def audit_pair(plan_path, folder):
    ...
    for arm, receipt in zip(order, completion['arms']):
        ...
        identity = load(target / 'memory_arm_receipt.json')
        ...
        if arm['geometry_storage'] is not None:
            actual_storage = target / 'archive_storage_receipt.json'
            assert identity['geometry_storage_sha256'] == sha(actual_storage)
            assert load(actual_storage)['geometry_storage'] == arm['geometry_storage']
            for role in ('novel','revisit'):
                status = load(target / 'evaluation' / role / 'memory_resources.json')['memory']
                assert status.get('geometry_storage','dense') == arm['geometry_storage']
                assert status['frames'] == status['online_depth_frames']
            sources[str(actual_storage)] = sha(actual_storage)
        assert receipt['receipt_sha256'] == sha(target / 'memory_arm_receipt.json')
        ...
        for name in ('memory_arm_receipt.json','fixed_window_receipt.json',
...
def run(args):
    ...
    for dataset in ('all','hm3d','mp3d'):
        for role in ('novel','revisit'):
            ...
            for control in ('legacy','native64'):
                comparisons[key]['support64_vs_' + control] = dict(
                    gained=sum(p['support64']['reached'] and not p[control]['reached'] for p in paired),
                    lost=sum(p[control]['reached'] and not p['support64']['reached'] for p in paired),
                    sr=clustered_difference(paired, 'support64', control, 'reached'),
                    spl=clustered_difference(paired, 'support64', control, 'spl'))
    report = dict(complete=complete, planned_pairs=70, verified_pairs=len(rows)//6,
        ...
        scope='Same-GPU fixed-W64 historical-storage comparison and paper GEM control on the full previously used population; scene-cluster bootstrap is descriptive, not a prespecified noninferiority test')
