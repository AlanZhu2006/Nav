#!/usr/bin/env bash
# One-scene, four-arm closed-loop evaluation for certified relocalization.
# A single MemNav/NavDP server pair is shared by every arm.  Goal-A is sampled
# once and replayed byte-identically, so only Goal-B routing/controller behavior
# can differ.
...
for row in manifest["episodes"][scene]:
    ...
    for kind, path in paths.items():
        if sha(path) != row["files"][kind]["sha256"]:
            raise SystemExit(
                f"episode identity changed: {scene}/{row['episode']}/{kind}")
    episodes.append(row["episode"])
if len(episodes) != 8:
    raise SystemExit("formal closed loop requires eight episodes per scene")
print(scene)
print(asset)
print(episode_root)
print(",".join(episodes))
...
# Four-row Williams design: every arm occurs equally often in every position
# and first-order carry-over is balanced when repeated over twenty scenes.
case $((10#${SCENE_INDEX} % 4)) in
  0) arm_order=(certified_relocalization known_revisit_direct native geometry_router) ;;
  1) arm_order=(known_revisit_direct geometry_router certified_relocalization native) ;;
  2) arm_order=(geometry_router native known_revisit_direct certified_relocalization) ;;
  3) arm_order=(native certified_relocalization geometry_router known_revisit_direct) ;;
esac
...
with open(path, "x", encoding="utf-8") as handle:
    json.dump({
        ...
        "arm_order": order,
        "primary_contrast": "certified_relocalization_minus_native",
...
for arm in (
    "trace_source", "certified_relocalization", "known_revisit_direct",
    "geometry_router", "native",
):
