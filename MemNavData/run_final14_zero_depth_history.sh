#!/usr/bin/env bash
# One frozen Final14 history, one explicit-zero native arm, two hidden roles.
set -euo pipefail
umask 0022
...
EXPECTED_REFERENCE_SUMMARY_SHA=ae24138b3cd7fecb737dffe8454eb91aaae8f5aa19d4e679e67e039d793e17b7
EXPECTED_REFERENCE_VERIFY_SHA=7bf7e496c1a9cc53f3dc9ef0ff0194cce61f03d554a84678210320f699fbb35f
...
[[ "$(sha256sum "${REFERENCE_ROOT}/POSTHOC/final14_mono_factorial_summary.json" | awk '{print $1}')" == \
   "${EXPECTED_REFERENCE_SUMMARY_SHA}" ]]
[[ "$(sha256sum "${REFERENCE_ROOT}/POSTHOC/final14_mono_factorial_independent_verification.json" | awk '{print $1}')" == \
   "${EXPECTED_REFERENCE_VERIFY_SHA}" ]]
...
# Socket readiness is insufficient when a model server is half initialized.
# A reset-only transaction changes no evaluated observation and is repeated by
# the evaluator immediately afterwards.
...
runner=("${REPAIR_ROOT}/MemNavData/run_final14_zero_depth_episode.py"
  --source-root "${BASE_SOURCE_ROOT}" --run-root "${RUN_ROOT}"
  --reference-root "${REFERENCE_ROOT}" --bench-root "${BENCH_ROOT}"
...
PYTHONPATH="${REPAIR_ROOT}:${BASE_SOURCE_ROOT}:${HAB_SITE}/pip/_vendor" \
  "${HAB_PY}" -u "${runner[@]}" \
