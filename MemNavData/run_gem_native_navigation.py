"""Fixed native W16/W64 and legacy navigation on the original 70 histories.
...
ARMS = (
    dict(name='legacy', mode='legacy', dense_window=32),
    dict(name='native64', mode='native_interval7', dense_window=64),
    dict(name='native16', mode='native_interval7', dense_window=16))
...
def check_plan(path):
    plan = load(path)
    assert plan['schema'] == 'gem_native_navigation_paired_v1'
...
def freeze(args):
    ...
    plan = dict(schema='gem_native_navigation_paired_v1',
        ...
        scope='Complete previously used development population; new same-GPU navigation comparison, not new unseen scenes or a novel memory mechanism',
        production='API002, native window is the sole difference between native64/native16; SP/LG/PnP/controller unchanged',
...
def run_arm(args):
    ...
    save(args.out / 'native_arm_receipt.json', dict(completed=True, arm=arm,
...
def run_pair(args):
    ...
    try:
        for arm in order:
            ...
            rows.append(dict(arm=arm, receipt_sha256=sha(target / 'native_arm_receipt.json'),
