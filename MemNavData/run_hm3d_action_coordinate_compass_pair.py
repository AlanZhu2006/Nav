#!/usr/bin/env python3
"""Paired long-range evaluation of endpoint and action-coordinate CEC.

Both arms replay the same sealed causal RGB history in the same process.  The
runtime never receives Novel/Revisit labels.  Navigation outcomes are read only
after canonical endpoint-bearing CEC and the preregistered continuous
action-coordinate CEC arm have both completed.
"""
...
SCHEMA_VERSION = "hm3d_action_coordinate_compass_pair_v1_20260902"
ARMS = (
    "mono_cec_endpoint",
    "mono_cec_action_coordinate",
)
ARM_CONFIG = {
    "mono_cec_endpoint": {
    ...
    "mono_cec_action_coordinate": {
        "hybrid_route": "certified_relocalization",
        "revisit_adapter": "verified_bearing_v1",
        "guidance_mode": "action_coordinate_compass",
        "evaluator_arm": "certified",
        "depth_audit_arm": "mono_cec",
    },
}
...
def guidance_receipt_audit(
    ...
    require(all(
        ...
        == config["guidance_mode"] for plan in accepted
    ), f"{arm}/{role}: accepted request changed guidance mode")
    if arm == "mono_cec_action_coordinate":
        route = audit_action_coordinate_plans(plans)
    else:
        require(not any(
            plan.get("action_coordinate_compass_requested") is True
            for plan in accepted
        ), f"{arm}/{role}: endpoint arm entered action-coordinate guidance")
        route = None
    return {
        "depth": depth,
        "target_proof": proof,
...
def main() -> int:
    ...
    require(arguments.history_contract == "causal_survey",
            "paired compass evaluation requires causal-survey history")
    require(arguments.role_pair_scope == "table3_length",
            "paired compass evaluation requires the length population")
    ...
    require(prefix_a_steps >= 40 and prefix_b_steps == 0,
            "causal survey cannot establish first-40 receipt")
    ...
    output = (arguments.run_root / "development" /
              "action_coordinate_compass" / label)
    require(not output.exists(), f"paired output exists: {output}")
    ...
    contract = {
        ...
        "stuck_trigger_within_budget": False,
    ...
    endpoint_rows = rows_by_arm["mono_cec_endpoint"]
    endpoint_payloads = payloads_by_arm["mono_cec_endpoint"]
    for arm in ARMS:
        compare_shared_replay(
            endpoint_rows, endpoint_payloads,
            rows_by_arm[arm], payloads_by_arm[arm], arm)

    # The two arms have byte-identical proofs.  When that shared proof rejects,
    # both must therefore issue the same native request and physical trace.
    fallback: dict[str, bool] = {}
    for role in ("novel", "revisit"):
        fallback[role] = audit_fully_rejected_fallback(
            arm="mono_cec_action_coordinate",
            role=role,
            cec_row=rows_by_arm["mono_cec_action_coordinate"][role],
            cec_payload=payloads_by_arm[
                "mono_cec_action_coordinate"][role],
            native_payload=endpoint_payloads[role],
        )

    for role in ("novel", "revisit"):
        endpoint = audits_by_arm["mono_cec_endpoint"][role]["target_proof"]
        route = audits_by_arm[
            "mono_cec_action_coordinate"][role]["target_proof"]
    ...
    completion = {
        **contract,
        ...
        "shared_rejection_exact_request_and_trace": fallback,
