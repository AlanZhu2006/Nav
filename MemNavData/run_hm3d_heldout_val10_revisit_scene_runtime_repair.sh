#!/usr/bin/env bash
# Evaluate one frozen HM3D held-out scene with the verified NNR runtime.
...
RUN_ROOT=${RUN_ROOT:?set fresh formal run root}
MANIFEST=${MANIFEST:?set frozen data manifest}
EXPECTED_MANIFEST_SHA=${EXPECTED_MANIFEST_SHA:?set manifest SHA}
SCENE_INDEX=${SCENE_INDEX:?set original selected-scene index}
...
missing = required.difference(adapter.REVISIT_ADAPTER_MODES)
if missing:
    raise SystemExit(f"adapter overlay is incomplete: {sorted(missing)}")
PY
...
if index not in m["construction_attrition"]["evaluation_scene_indices"]:
    raise SystemExit("scene index is outside the frozen constructible set")
...
with open(path,"x",encoding="utf-8") as handle:
    json.dump({
      "schema_version":"hm3d_heldout_runtime_repair_scene_v1_20260816",
