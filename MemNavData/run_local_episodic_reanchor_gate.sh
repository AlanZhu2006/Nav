#!/usr/bin/env bash
# One real LingBot/CEC local visual re-anchor gate; no NavDP, actions, or SR.
set -euo pipefail
umask 0022
...
QUERY_SET=${QUERY_SET:-${ROOT}/.diagnostics/long_range_path_field_20260902/reanchor_queries_index${HISTORY_INDEX}_v2_attempt2/query_set.json}
OUT_ROOT=${OUT_ROOT:-${ROOT}/.diagnostics/long_range_path_field_20260902/local_reanchor_gate_index${HISTORY_INDEX}_v2_attempt1}
MEMNAV_PORT=${MEMNAV_PORT:-22671}
...
LOCAL_REANCHOR_CLIENT=${LOCAL_REANCHOR_CLIENT:-${ROOT}/MemNavData/run_local_episodic_reanchor_gate.py}

fail() { echo "ABORT: $*" >&2; exit 2; }
[[ "${HISTORY_INDEX}" =~ ^(0|16|32)$ ]] || fail "HISTORY_INDEX must be 0,16,32"
[[ ! -e "${OUT_ROOT}" ]] || fail "output exists: ${OUT_ROOT}"
for path in "${MANIFEST}" "${QUERY_SET}" "${MEMNAV_PY}" "${MEMNAV_CKPT}" \
  "${LINGBOT_WEIGHTS}" "${LIGHTGLUE_REPO}/lightglue" \
  "${DEPENDENCY_ROOT}/kornia" "${INTERNNAV_ROOT}" \
  "${LOCAL_REANCHOR_CLIENT}"; do
...
runtime_root=$(mktemp -d /tmp/epf_reanchor.XXXXXX)
MEMNAV_PID=
cleanup() {
...
PYTHONPATH="${ROOT}:${ROOT}/MemNavData" "${MEMNAV_PY}" -u \
  "${LOCAL_REANCHOR_CLIENT}" \
    ...
    --query-set "${QUERY_SET}" --history-index "${HISTORY_INDEX}" \
    --port "${MEMNAV_PORT}" --out "${OUT_ROOT}/result" \
    >"${OUT_ROOT}/logs/client.log" 2>&1

cat "${OUT_ROOT}/logs/client.log"
echo "LOCAL_REANCHOR_CLIENT_COMPLETE client=${LOCAL_REANCHOR_CLIENT} output=${OUT_ROOT}/result"
