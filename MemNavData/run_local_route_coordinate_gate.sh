#!/usr/bin/env bash
# Proof once, then track one continuous route coordinate. No NavDP/actions/SR.
set -euo pipefail
umask 0022
...
HISTORY_INDEX=16
QUERY_SET=${QUERY_SET:-${ROOT}/.diagnostics/long_range_path_field_20260902/route_coordinate_queries_index16_v2/query_set.json}
OUT_ROOT=${OUT_ROOT:-${ROOT}/.diagnostics/long_range_path_field_20260902/local_route_coordinate_gate_index16_attempt1}
MEMNAV_PORT=${MEMNAV_PORT:-22673}
...
runtime_root=$(mktemp -d /tmp/route_coordinate.XXXXXX)
MEMNAV_PID=
cleanup() {
...
PYTHONPATH="${ROOT}:${ROOT}/MemNavData" "${MEMNAV_PY}" -m py_compile \
  ...
  "${ROOT}/MemNavData/run_local_route_coordinate_gate.py" \
...
(
  ...
  exec env PYTHONUNBUFFERED=1 PYTHONDONTWRITEBYTECODE=1 \
    ...
    "${ROOT}/NavDP/baselines/memnav/memnav_server.py" \
      ...
      --certified_relocalization --certified_route_depth_cache_stride 8 \
...
echo "LOCAL_ROUTE_COORDINATE_GATE_COMPLETE output=${OUT_ROOT}/result/gate_result.json"
