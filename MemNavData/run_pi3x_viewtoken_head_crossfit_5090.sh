#!/usr/bin/env bash
# Scale-invariant cross-fitted reliability ensemble on the frozen Pi3X b16 tokens.
set -euo pipefail
umask 0022

readonly SOURCE_ROOT=/home/cv/memnav_eval/source_bundles/pi3x_viewtoken_head_v2_5d0838ba38eefbff
readonly SOURCE_RECEIPT=${SOURCE_ROOT}/SOURCE_BUNDLE.sha256
readonly EXPECTED_SOURCE_RECEIPT_SHA=5d0838ba38eefbffaa07e844ef0df4eed837bd70c3e48c4bdd1729fab8116a14
...
readonly TRAINER=${SOURCE_ROOT}/MemNavData/train_pi3x_viewtoken_reliability_crossfit_oof.py
...
[[ -f "${TRAINER}" && ! -e "${RESULT_ROOT}" ]] || fail "trainer missing or result root already exists"
...
"${PYTHON}" -m pytest -q -p no:cacheprovider \
  ...
  "${SOURCE_ROOT}/MemNavData/test_train_pi3x_viewtoken_reliability_crossfit_oof.py" \
...
"${PYTHON}" -u "${TRAINER}" "${COMMON[@]}" \
  --outer-splits 5 --inner-splits 4 --consensus 3 --epochs 30 \
