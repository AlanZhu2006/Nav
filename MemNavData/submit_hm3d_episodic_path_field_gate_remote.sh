...
RUN_ROOT=${RUN_ROOT:-/scratch/yz11502/Research/Nav-axis-uturn-results/hm3d_episodic_path_field_20260902/gate_${SOURCE_SHA:0:16}}
...
# Import the exact evaluator closure in the same container and interpreter as
# the job before consuming a GPU allocation.
HAB_SITE=$("${HAB_PY}" -c \
...
SBATCH=${SOURCE_ROOT}/MemNavData/slurm_hm3d_episodic_path_field_gate.sbatch
...
safe_sbatch --lint-fatal --test-only \
  ...
  --cpus-per-task=10 --mem=72G --time=01:00:00 --array=0,16,32%3 \
  --export="${exports}" "${SBATCH}" >/dev/null
if [[ "${DRY_RUN}" == 1 ]]; then
  echo "DRY_RUN_OK indices=0,16,32 time=01:00:00 source=${SOURCE_SHA}"
  exit 0
fi

mkdir -p "${RUN_ROOT}"
job=$(safe_sbatch --lint-fatal --parsable --job-name=h3EpfGate \
  --partition=h100_tandon,a100_tandon \
  --account=torch_pr_769_tandon_advanced --qos=gpu48 --gres=gpu:1 \
  --cpus-per-task=10 --mem=72G --time=01:00:00 --array=0,16,32%3 \
...
"/scratch/lg154/conda-envs/memnav/bin/python" - \
  "${RUN_ROOT}/submission.json" "${job}" "${SOURCE_ROOT}" "${SOURCE_SHA}" <<'PY'
import datetime,json,os,sys
path,job,source,source_sha=sys.argv[1:]
payload={
 'schema_version':'hm3d_episodic_path_field_gate_submission_v1_20260902',
 'submitted_at':datetime.datetime.now(datetime.timezone.utc).isoformat(),
 'job_id':int(job), 'array_indices':[0,16,32], 'array_concurrency':3,
 'partition':['h100_tandon','a100_tandon'], 'qos':'gpu48',
 'requested_time_per_element':'01:00:00', 'query_step_guard':80,
 ...
 'stage':'LingBot-coordinate deployment gate',
