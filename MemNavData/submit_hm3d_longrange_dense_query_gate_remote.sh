...
RUN_ROOT=${RUN_ROOT:-/scratch/yz11502/Research/Nav-axis-uturn-results/hm3d_longrange_dense_live_query_gate_20260903/consumed_${SOURCE_SHA:0:16}}
DRY_RUN=${DRY_RUN:-0}
...
PAIR_SBATCH=${SOURCE_ROOT}/MemNavData/slurm_hm3d_longrange_dense_query_gate.sbatch
RESULT_SBATCH=${SOURCE_ROOT}/MemNavData/slurm_hm3d_longrange_dense_query_gate_result.sbatch
...
safe_sbatch --lint-fatal --test-only --partition=h100_tandon,a100_tandon \
  ...
  --cpus-per-task=10 --mem=72G --time="${PAIR_TIME}" --array="${SELECTED}%3" \
...
mkdir -p "${RUN_ROOT}"
eval_raw=$(safe_sbatch --lint-fatal --parsable \
  --partition=h100_tandon,a100_tandon \
  --account=torch_pr_769_tandon_advanced --qos=gpu48 --gres=gpu:1 \
  --cpus-per-task=10 --mem=72G --time="${PAIR_TIME}" \
  --array="${SELECTED}%3" --export="${common}" "${PAIR_SBATCH}")
eval_job=$(printf '%s\n' "${eval_raw}" | job_id)
[[ "${eval_job}" =~ ^[0-9]+$ ]] || fail "invalid evaluation job id"
analysis_raw=$(safe_sbatch --lint-fatal --parsable --partition=cpu_short \
  --time=00:10:00 --dependency="afterok:${eval_job}" \
...
/scratch/lg154/conda-envs/memnav/bin/python - \
  "${RUN_ROOT}/submission.json" "${eval_job}" "${analysis_job}" \
  "${verify_job}" "${SOURCE_ROOT}" "${SOURCE_SHA}" "${PAIR_TIME}" <<'PY'
import datetime, json, os, sys
path,evaluation,analysis,verify,source,source_sha,pair_time=sys.argv[1:]
payload={
 'schema_version':'hm3d_longrange_dense_live_query_gate_submission_v2_20260903',
 'submitted_at':datetime.datetime.now(datetime.timezone.utc).isoformat(),
 'evaluation_job':int(evaluation), 'analysis_job':int(analysis),
 'result_verification_job':int(verify), 'selected_history_indices':[0,1,6],
 'array_concurrency':3, 'partition':['h100_tandon','a100_tandon'],
 'qos':'gpu48', 'requested_time_per_element':pair_time,
 'source_root':source, 'source_receipt_sha256':source_sha,
 'claim_scope':'consumed implementation mechanism gate only',
 'motion_model':'direct_pnp_dense_query',
 'navigation_claim_allowed':False, 'fresh_confirmation_required':True,
 'parent_navigation_outcomes_read_before_submission':True,
 'threshold_search_allowed':False, 'radius_search_allowed':False,
}
...
printf 'EVALUATION_JOB=%s\nANALYSIS_JOB=%s\nVERIFY_JOB=%s\nRUN_ROOT=%s\n' \
  "${eval_job}" "${analysis_job}" "${verify_job}" "${RUN_ROOT}"
