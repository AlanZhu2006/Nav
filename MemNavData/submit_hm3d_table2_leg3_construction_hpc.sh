#!/usr/bin/env bash
# Freeze and submit only the new Table-2 Leg-3 query construction DAG.
set -euo pipefail
umask 0022
...
remote() {
  # Closing stdin is mandatory for a forced-PTY call inside command
  # substitution; otherwise the local shell can stop on SIGTTIN and look like
  # a remote/HPC hang.
...
files=(
  ...
  MemNavData/hm3d_table2_leg3_construction_closure_repair_20260829.json
  ...
  MemNavData/submit_hm3d_table2_leg3_construction_hpc.sh
)
...
"${LOCAL_MEMNAV_PY}" -m json.tool \
  MemNavData/hm3d_table2_leg3_mixed_role_protocol_20260829.json >/dev/null
...
task_root=${REMOTE_BUNDLES}/hm3d_table2_leg3_${bundle_key}
...
test \"\$(sha256sum '${SOURCE_RUN_ROOT}/population/population.json' | awk '{print \$1}')\" = ec11c0dbc43a4abe585330c1ce52a8c14ad1d4b1da6fd8397e1d15592707a6d5
test \"\$(sha256sum '${SOURCE_RUN_ROOT}/population/SEALED' | awk '{print \$1}')\" = 9c0410578381602eeed64ebebc56acc0a1a36add11f37795553460d9376daf79
...
mkdir -p '${run_root}/construction/fragments' '${run_root}/logs' '${run_root}/sealed_inputs' '${smoke_root}/construction/fragments' '${smoke_root}/logs' /scratch/yz11502/Research/Nav-axis-uturn-results/slurm_logs
cp '${protocol}' '${run_root}/sealed_inputs/'
sha256sum '${SOURCE_RUN_ROOT}/population/population.json' '${SOURCE_RUN_ROOT}/population/SEALED' >'${run_root}/sealed_inputs/source_inputs.sha256'
chmod -R a-w '${run_root}/sealed_inputs'"

remote "singularity exec --nv -B /scratch/lg154 -B /scratch/yz11502 /share/apps/images/cuda12.8.1-cudnn9.8.0-ubuntu24.04.2.sif env PYTHONDONTWRITEBYTECODE=1 PYTHONPATH='${task_root}:${task_root}/MemNavData:${BASE_SOURCE_ROOT}:${BASE_SOURCE_ROOT}/MemNavData' /scratch/lg154/conda-envs/habitat/bin/python -c 'import construct_hm3d_table2_leg3_mixed_role as c; c.validate_dependency_contract()' && env PYTHONDONTWRITEBYTECODE=1 PYTHONPATH='${task_root}:${task_root}/MemNavData:${BASE_SOURCE_ROOT}:${BASE_SOURCE_ROOT}/MemNavData' /scratch/lg154/conda-envs/memnav/bin/python -m pytest -q '${task_root}/MemNavData/test_hm3d_table2_leg3_mixed_role.py'"
...
remote "sbatch --test-only --partition=h100_tandon,a100_tandon --account=torch_pr_769_tandon_advanced --qos=gpu48 --gres=gpu:1 --time=00:45:00 --array=0 --export='${common},RUN_ROOT=${smoke_root}' '${construct}' >/dev/null"
remote "sbatch --test-only --partition=h100_tandon,a100_tandon --account=torch_pr_769_tandon_advanced --qos=gpu48 --gres=gpu:1 --time=00:45:00 --array=0-21%${CONSTRUCT_CONCURRENCY} --export='${common}' '${construct}' >/dev/null"
...
smoke_raw=$(remote "sbatch --parsable --job-name=h3T2L3smoke --partition=h100_tandon,a100_tandon --account=torch_pr_769_tandon_advanced --qos=gpu48 --gres=gpu:1 --time=00:45:00 --array=0 --export='${common},RUN_ROOT=${smoke_root}' '${construct}'" | tr -d '\r')
smoke_id=${smoke_raw%%;*}; [[ "${smoke_id}" =~ ^[0-9]+$ ]] || fail "bad smoke job"
formal_raw=$(remote "sbatch --parsable --job-name=h3T2L3build --partition=h100_tandon,a100_tandon --account=torch_pr_769_tandon_advanced --qos=gpu48 --gres=gpu:1 --time=00:45:00 --array=0-21%${CONSTRUCT_CONCURRENCY} --dependency=afterok:${smoke_id} --kill-on-invalid-dep=yes --export='${common}' '${construct}'" | tr -d '\r')
formal_id=${formal_raw%%;*}; [[ "${formal_id}" =~ ^[0-9]+$ ]] || fail "bad formal array"
finalize_raw=$(remote "sbatch --parsable --job-name=h3T2L3final --partition=cpu_short --account=torch_pr_769_tandon_advanced --time=00:45:00 --dependency=afterok:${formal_id} --kill-on-invalid-dep=yes --export='${common},MODE=finalize' '${analysis}'" | tr -d '\r')
finalize_id=${finalize_raw%%;*}; [[ "${finalize_id}" =~ ^[0-9]+$ ]] || fail "bad finalizer"
verify_raw=$(remote "sbatch --parsable --job-name=h3T2L3verify --partition=cpu_short --account=torch_pr_769_tandon_advanced --time=00:45:00 --dependency=afterok:${finalize_id} --kill-on-invalid-dep=yes --export='${common},MODE=verify' '${analysis}'" | tr -d '\r')
verify_id=${verify_raw%%;*}; [[ "${verify_id}" =~ ^[0-9]+$ ]] || fail "bad verifier"

receipt=MemNavData/HM3D_TABLE2_LEG3_CONSTRUCTION_SUBMISSION_20260829.json
...
payload={
 "schema_version":"hm3d_table2_leg3_construction_submission_v1_20260829",
 "scope":"new-query construction only; no Leg-3 policy rollout",
