...
REMOTE_RESULTS=${REMOTE_RESULTS:-/scratch/yz11502/Research/Nav-axis-uturn-results/hm3d_table3_navmesh_capacity_20260830}
PARENT_ROOT=${PARENT_ROOT:-/scratch/yz11502/Research/Nav-axis-uturn-results/hm3d_fresh_fullmono_mixed_role_20260820/formal_20260820T143609Z_e6dd44c6}
PARENT_MANIFEST_LOCAL=${PARENT_MANIFEST_LOCAL:-}
PARENT_MANIFEST=${PARENT_MANIFEST:-${PARENT_ROOT}/sealed_inputs/parent_manifest.json}
...
PROTOCOL_REL=${PROTOCOL_REL:-MemNavData/hm3d_table3_navmesh_capacity_protocol_20260830.json}
SCENE_COUNT=${SCENE_COUNT:-54}
BUNDLE_PREFIX=${BUNDLE_PREFIX:-hm3d_table3_capacity}
RUN_PREFIX=${RUN_PREFIX:-capacity}
LOCAL_SUBMISSION_RECEIPT=${LOCAL_SUBMISSION_RECEIPT:-MemNavData/HM3D_TABLE3_NAVMESH_CAPACITY_SUBMISSION_20260830.json}
...
[[ "${SCENE_COUNT}" =~ ^[1-9][0-9]*$ ]] || fail "invalid scene count"
...
if [[ -n "${PARENT_MANIFEST_LOCAL}" ]]; then
  files+=("${PARENT_MANIFEST_LOCAL}")
  [[ "$(sha256sum "${PARENT_MANIFEST_LOCAL}" | awk '{print $1}')" == \
     "${EXPECTED_PARENT_MANIFEST_SHA}" ]] || fail "local parent manifest changed"
fi
...
bundle=${REMOTE_BUNDLES}/${BUNDLE_PREFIX}_${receipt_sha:0:16}
stage=${bundle}.partial.$$
run=${REMOTE_RESULTS}/${RUN_PREFIX}_$(date -u +%Y%m%dT%H%M%SZ)_${receipt_sha:0:8}
...
if [[ -n "${PARENT_MANIFEST_LOCAL}" ]]; then
  parent_manifest_remote=${bundle}/MemNavData/$(basename "${PARENT_MANIFEST_LOCAL}")
else
  parent_manifest_remote=${PARENT_MANIFEST}
fi
remote "test \"\$(sha256sum '${parent_manifest_remote}' | awk '{print \$1}')\" = '${EXPECTED_PARENT_MANIFEST_SHA}'" >/dev/null
...
common_base="ALL,TASK_ROOT=${bundle},TASK_RECEIPT=${receipt},EXPECTED_TASK_RECEIPT_SHA=${receipt_sha},PARENT_ROOT=${PARENT_ROOT},PARENT_MANIFEST_PATH=${parent_manifest_remote},PROTOCOL=${protocol},EXPECTED_PROTOCOL_SHA=${protocol_sha},EXPECTED_PARENT_MANIFEST_SHA=${EXPECTED_PARENT_MANIFEST_SHA}"
...
last=$((SCENE_COUNT - 1))
array=$(remote "source '${bundle}/MemNavData/slurm_safe_submit.sh'; safe_sbatch --lint-fatal --parsable --partition=cpu_short --array=0-${last}%${CONCURRENCY} --dependency=afterok:${smoke} --kill-on-invalid-dep=yes --export='${formal_common}' '${scene_script}'" | job_id)
...
python - "${submission}" "${bundle}" "${receipt_sha}" "${run}" \
  "${smoke}" "${array}" "${finalize}" "${verify}" "${SCENE_COUNT}" <<'PY'
import json,sys
out,bundle,sha,run,smoke,array,finalize,verify,scene_count=sys.argv[1:]
p={
 ...
 'scene_array_job':int(array),'scene_array':f'0-{int(scene_count)-1}',
...
cp -p "${submission}" "${LOCAL_SUBMISSION_RECEIPT}"
printf 'RUN_ROOT=%s\nBUNDLE=%s\nSMOKE_JOB=%s\nARRAY_JOB=%s\nFINALIZE_JOB=%s\nVERIFY_JOB=%s\n' \
