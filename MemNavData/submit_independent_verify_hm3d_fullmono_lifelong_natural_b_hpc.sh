#!/usr/bin/env bash
# Submit a read-only verifier after the sealed Natural-B construction audit.
set -euo pipefail
umask 0022

ROOT=${ROOT:-/home/asus/Research/Nav-graph-blind}
SSH_ALIAS=${SSH_ALIAS:-alantorch}
SEAL_JOB_ID=${SEAL_JOB_ID:-16464407}
RUN_ROOT=${RUN_ROOT:-/scratch/yz11502/Research/Nav-axis-uturn-results/hm3d_fullmono_lifelong_power_v3_20260826/natural_b_audit_formal_20260827T125748Z_e2832e17}
...
files=(
  MemNavData/independent_verify_hm3d_fullmono_lifelong_natural_b.py
  MemNavData/test_independent_verify_hm3d_fullmono_lifelong_natural_b.py
  MemNavData/slurm_independent_verify_hm3d_fullmono_lifelong_natural_b.sbatch
...
bash -n \
  MemNavData/slurm_independent_verify_hm3d_fullmono_lifelong_natural_b.sbatch \
  MemNavData/slurm_safe_submit.sh \
  MemNavData/submit_independent_verify_hm3d_fullmono_lifelong_natural_b_hpc.sh
(
...
scratch=$(mktemp -d /tmp/h3life_natb_verify_submit.XXXXXX)
...
bundle=${REMOTE_BUNDLES}/hm3d_lifelong_natural_b_verifier_${receipt_sha:0:16}
...
script=${bundle}/MemNavData/slurm_independent_verify_hm3d_fullmono_lifelong_natural_b.sbatch
common="ALL,SOURCE_ROOT=${bundle},SOURCE_RECEIPT=${source_receipt},EXPECTED_SOURCE_RECEIPT_SHA=${receipt_sha},RUN_ROOT=${RUN_ROOT}"
remote "test -d '${RUN_ROOT}' && scontrol show job '${SEAL_JOB_ID}' >/dev/null"
...
printf '{\n  "schema_version": "hm3d_fullmono_lifelong_natural_b_verifier_submission_v1_20260827",\n  "scope": "read-only independent constructibility recount",\n  "run_root": "%s",\n  "source_bundle": "%s",\n  "source_bundle_receipt_sha256": "%s",\n  "seal_job": %s,\n  "verifier_job": %s,\n  "dependency": "afterok:%s",\n  "navigation_rollouts_authorized": false,\n  "evaluation_authorized": false\n}\n' \
  "${RUN_ROOT}" "${bundle}" "${receipt_sha}" "${SEAL_JOB_ID}" \
  "${verify_id}" "${SEAL_JOB_ID}" \
  >MemNavData/HM3D_FULLMONO_LIFELONG_NATURAL_B_VERIFIER_SUBMISSION_20260827.json
