...
# for the MDTEC monocular x CEC composition experiment.
#
# PRECONDITION (protocol-enforced, not just documentation): this must not
# be submitted before Gate D's independent verifier posts a formal pass --
# see MDTEC_MONOCULAR_CEC_COMPOSITION_PROTOCOL_20260819.md, "status:
# conditional_on_gate_d_authorization". This script does not check that
# for you; it is a deliberate human checkpoint.
set -euo pipefail
umask 0022
...
REMOTE_RESULTS=/scratch/yz11502/Research/Nav-axis-uturn-results/mdtec_monocular_cec_composition_20260819
...
"${LOCAL_PY}" -m py_compile \
  ...
  MemNavData/mdtec_monocular_cec_composition.py \
  MemNavData/eval_mdtec_monocular_cec_composition_habitat.py \
  MemNavData/summarize_mdtec_monocular_cec_composition.py \
  MemNavData/independent_verify_mdtec_monocular_cec_composition.py \
...
bash -n MemNavData/run_mdtec_monocular_cec_composition_scene.sh \
  MemNavData/slurm_mdtec_monocular_cec_composition.sbatch \
  MemNavData/slurm_mdtec_monocular_cec_composition_summary.sbatch

staging=$(mktemp -d)
cleanup() { rm -rf -- "${staging}"; }
trap cleanup EXIT
files=(
  MemNavData/MDTEC_MONOCULAR_CEC_COMPOSITION_PROTOCOL_20260819.md
  MemNavData/mdtec_monocular_cec_composition_protocol_20260819.json
  MemNavData/monocular_depth_runtime.py
  MemNavData/mdtec_raw_depth_gate_d.py
  MemNavData/mdtec_monocular_cec_composition.py
  MemNavData/eval_mdtec_monocular_cec_composition_habitat.py
  ...
  MemNavData/test_mdtec_monocular_cec_composition.py
  ...
  MemNavData/run_mdtec_monocular_cec_composition_scene.sh
  MemNavData/slurm_mdtec_monocular_cec_composition.sbatch
  MemNavData/slurm_mdtec_monocular_cec_composition_summary.sbatch
)
...
bundle_tag=mdtec_cec_composition_${receipt_sha:0:16}
...
array_job=$(ssh "${SSH_ALIAS}" "sbatch --parsable --array=0-19%10 --export=ALL,SOURCE_ROOT='${remote_root}',RUN_ROOT='${run_root}',SOURCE_RECEIPT='${remote_root}/source_inputs.sha256',EXPECTED_SOURCE_RECEIPT_SHA='${receipt_sha}' '${remote_root}/MemNavData/slurm_mdtec_monocular_cec_composition.sbatch'")
summary_job=$(ssh "${SSH_ALIAS}" "sbatch --parsable --dependency=afterok:${array_job} --export=ALL,SOURCE_ROOT='${remote_root}',RUN_ROOT='${run_root}',SOURCE_RECEIPT='${remote_root}/source_inputs.sha256',EXPECTED_SOURCE_RECEIPT_SHA='${receipt_sha}' '${remote_root}/MemNavData/slurm_mdtec_monocular_cec_composition_summary.sbatch'")
