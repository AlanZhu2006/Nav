#!/usr/bin/env bash
# Freeze, upload, and submit smoke -> formal train40 arrival-consensus audits.
...
"${LOCAL_PY}" -m unittest -v \
  MemNavData.test_audit_navdp_arrival_consensus
...
remote "test -f /scratch/yz11502/Research/Nav-axis-uturn/.diagnostics/unseen_scene_eval_20260803/checkpoints/navdp_checkpoint.ckpt"
...
stage=$(mktemp -d /tmp/navdp_arrival_consensus.XXXXXX)
remote_partial=${REMOTE_BUNDLE_BASE}/navdp_arrival_consensus.partial.$$
cleanup() { rm -rf -- "${stage}"; }
trap cleanup EXIT
for relative in "${files[@]}"; do
...
root = out.parent
payload = {
    "schema_version": "navdp_arrival_consensus_bundle_v1_20260815",
    "created_utc": datetime.datetime.now(datetime.timezone.utc).isoformat(),
    "scope": "train-only NavDP candidate-consensus arrival audit",
...
remote_bundle=${REMOTE_BUNDLE_BASE}/navdp_arrival_consensus_${source_receipt_sha:0:16}
...
smoke_root=${REMOTE_RESULT_BASE}/smoke_${run_tag}
formal_root=${REMOTE_RESULT_BASE}/formal_${run_tag}
...
submit_one() {
  ...
  local run_root=$3
  local dependency=${4:-}
  local exports="ALL,SOURCE_ROOT=${remote_bundle},SOURCE_RECEIPT=${remote_bundle}/SOURCE_BUNDLE.sha256,EXPECTED_SOURCE_RECEIPT_SHA=${source_receipt_sha},BASE_SOURCE_ROOT=${BASE_SOURCE_ROOT},EXPECTED_BASE_SOURCE_RECEIPT_SHA=${EXPECTED_BASE_SOURCE_RECEIPT_SHA},RUN_ROOT=${run_root},MODE=${mode},MAX_EPISODES=${max_episodes}"
  ...
  local raw
  raw=$(remote "sbatch --begin=now+5minutes ${dependency_arg} --parsable --export='${exports}' '${launcher}'")
  local job=${raw%%;*}
  [[ "${job}" =~ ^[0-9]+$ ]] || fail "bad Slurm job id: ${raw}"
  remote "'${REMOTE_PY}' - '${run_root}/submission.json' '${job}' '${mode}' '${max_episodes}' '${remote_bundle}' '${source_receipt_sha}' '${BASE_SOURCE_ROOT}' '${EXPECTED_BASE_SOURCE_RECEIPT_SHA}'" <<'PY'
import json
import sys
import time
path, job, mode, max_episodes, bundle, receipt, base, base_receipt = sys.argv[1:]
payload = {
    "schema_version": "navdp_arrival_consensus_submission_v1_20260815",
    "job_id": int(job),
    "mode": mode,
...
smoke_job=$(submit_one smoke 1 "${smoke_root}")
formal_job=$(submit_one formal 0 "${formal_root}" "${smoke_job}")

echo "smoke_job_id=${smoke_job}"
echo "formal_job_id=${formal_job}"
