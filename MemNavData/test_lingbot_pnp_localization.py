...
def test_reference_superpoint_cache_is_bounded_and_file_versioned(tmp_path):
    matcher = object.__new__(LightGluePointMatcher)
    matcher.device = torch.device("cpu")
    matcher.reference_cache_size = 2
    matcher._reference_feature_cache = OrderedDict()
    calls = []
    ...
    def extract(image):
        calls.append(("extract", tuple(image.shape)))
        return {"feature": torch.tensor([len(calls)])}

    matcher.load_image = load_image
    matcher._extract = extract
    ...
    value1 = matcher._reference_features(first)
    value2 = matcher._reference_features(first)
    assert value1 is value2
    assert [name for name, _ in calls].count("extract") == 1

    matcher._reference_features(second)
    matcher._reference_features(third)
    assert len(matcher._reference_feature_cache) == 2
    matcher._reference_features(first)
    assert [name for name, _ in calls].count("extract") == 4

    # Same pathname with new immutable contents must not reuse stale features.
    first.write_bytes(b"new-and-different")
    matcher._reference_features(first)
    assert [name for name, _ in calls].count("extract") == 5
