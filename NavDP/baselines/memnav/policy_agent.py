...
class MemNavAgent:
    ...
    def _certified_reference_depth(self, anchor):
        """Replay one selected anchor and return its causal LingBot depth.

        Candidate ranking is image-only, so this expensive full replay happens
        for at most one history frame per goal.  Starting immediately after
        the frozen scale block matches the independently confirmed collector;
        the streaming KV state is restored even when inference fails.
        """
        anchor = int(anchor)
