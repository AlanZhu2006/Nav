...
def vint_reset():
    ...
    if args.record_video and vint_fps_writer is None:
...
def vint_observation_step():
    """Advance ViNT's short RGB context without producing an action."""
    global vint_navigator
    image_file = request.files['image']
    image = Image.open(image_file.stream).convert('RGB')
    image = np.asarray(image)
    image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
    image = image.reshape((vint_navigator.batch_size, -1, image.shape[1], 3))
    vint_navigator.observe(image)
    return jsonify({"algo": "vint", "observed": True})
...
def vint_step_nogoal():
    ...
    if vint_fps_writer is not None:
